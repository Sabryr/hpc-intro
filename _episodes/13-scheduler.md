...
```
[remote]$ srun {{ site.job_minimals_srun }} hostname
```
{: .bash}
```
gra752
...
```
[remote]$ srun {{ site.job_minimals_srun }} -c 2 echo "This job will use 2 CPUs."
```
{: .bash}
```
...
```
[remote]$ srun {{ site.job_minimals_srun }} --x11 --pty bash
```
{: .bash}
